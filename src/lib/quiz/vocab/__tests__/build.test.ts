...
import {
  TOEIC500_F_UNITS,
  TOEIC500_F_VOCAB,
} from "@/lib/quiz/vocab/pack_toeic500_f";
import {
  TOEIC500_G_UNITS,
  TOEIC500_G_VOCAB,
} from "@/lib/quiz/vocab/pack_toeic500_g";
import {
  TOEIC500_H_UNITS,
  TOEIC500_H_VOCAB,
} from "@/lib/quiz/vocab/pack_toeic500_h";
...
describe("buildVocabUnits（TOEIC500）", () => {
  ...
  test("メタどおりの単元数・id・教科・バンド", () => {
    // A(1-3)+…+G(53-64)+H(65-70) の70単元。
    expect(units.map((u) => u.id)).toEqual(
      Array.from({ length: 70 }, (_, i) => `eikaiwa-500-${i + 1}`),
    );
